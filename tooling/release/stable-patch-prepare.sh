...
function set_cmake_version {
    sed -i "s/\(project(KDisplay VERSION \).*)/\1${1})/g" CMakeLists.txt
}

# We start from master (that has the beta tag).
git checkout master

# Something like: kdisplay@5.XX.0
LAST_TAG=$(git describe --abbrev=0)
LAST_BETA_VERSION=$(echo $LAST_TAG | sed -e 's,kdisplay@\(\),\1,g')
...
LAST_STABLE_VERSION=$(echo $LAST_STABLE_TAG | sed -e 's,kdisplay@\(\),\1,g')
...
standard-version -t kdisplay\@ --skip.commit true --skip.tag true --preMajor --release-as $PATCH_VERSION
...
git tag -a "kdisplay@${PATCH_VERSION}" -m "Create patch release ${PATCH_VERSION}."
