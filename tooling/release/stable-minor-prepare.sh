...
CURRENT_BETA_VERSION=$(echo $LAST_TAG | sed -e 's,kdisplay@\(\),\1,g')
...
standard-version -t kdisplay\@ --skip.commit true --skip.tag true --preMajor --release-as $RELEASE_VERSION
...
git tag -a "kdisplay@${RELEASE_VERSION}" -m "Create minor release ${RELEASE_VERSION}."
